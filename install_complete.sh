...
# Method 3: Look for extracted folder
    ...
    if [ -n "$EXTRACTED_DIRS" ]; then
        ...
        if command -v git &> /dev/null; then
            ...
            if git clone https://github.com/AbdelmonemAwad/ai-translator1.git "$INSTALL_DIR/temp_repo"; then
                SOURCE_DIR="$INSTALL_DIR/temp_repo"
            ...
            print_info "Git not found, using alternative download method..."
            SOURCE_DIR=""
        fi
        
        # Alternative download method if git fails
        if [ -z "$SOURCE_DIR" ]; then
            ...
            if command -v wget &> /dev/null; then
                ...
                if wget -q https://github.com/AbdelmonemAwad/ai-translator1/archive/refs/heads/main.zip; then
                    print_status "Download successful"
                    if command -v unzip &> /dev/null; then
                        unzip -q main.zip
                        SOURCE_DIR="$TMP_DIR/ai-translator1-main"
                    else
                ...
                if curl -s -L -o main.zip https://github.com/AbdelmonemAwad/ai-translator1/archive/refs/heads/main.zip; then
                    print_status "Download successful"
                    if command -v unzip &> /dev/null; then
                        unzip -q main.zip
                        SOURCE_DIR="$TMP_DIR/ai-translator1-main"
                ...
                print_error "Neither git, wget, nor curl found. Cannot download repository."
...
# Clean up temporary repository if it exists
if [ -d "$INSTALL_DIR/temp_repo" ]; then
    rm -rf "$INSTALL_DIR/temp_repo"
fi
...
if command -v apt &> /dev/null; then
    ...
    # Use existing Python instead of trying to install Python 3.11
    # This avoids the network error with PPA
    print_info "Using existing Python installation"
...
systemctl enable postgresql || print_warning "Failed to enable PostgreSQL service, but continuing"
systemctl start postgresql || print_warning "Failed to start PostgreSQL service, but continuing"

# Create database and user with better error handling
print_info "Creating AI Translator database..."
if sudo -u postgres psql -c "DROP DATABASE IF EXISTS ai_translator;" 2>/dev/null; then
    print_status "Dropped existing database if it existed"
fi

if sudo -u postgres psql -c "DROP USER IF EXISTS ai_translator;" 2>/dev/null; then
    print_status "Dropped existing user if it existed"
fi

if sudo -u postgres psql -c "CREATE DATABASE ai_translator;"; then
    print_status "Created database ai_translator"
else
    print_warning "Failed to create database, but continuing"
fi

if sudo -u postgres psql -c "CREATE USER ai_translator WITH PASSWORD 'ai_translator_pass2024';"; then
    print_status "Created user ai_translator"
else
    print_warning "Failed to create user, but continuing"
fi

sudo -u postgres psql -c "ALTER ROLE ai_translator SET client_encoding TO 'utf8';" || print_warning "Failed to set client encoding"
sudo -u postgres psql -c "ALTER ROLE ai_translator SET default_transaction_isolation TO 'read committed';" || print_warning "Failed to set transaction isolation"
sudo -u postgres psql -c "ALTER ROLE ai_translator SET timezone TO 'UTC';" || print_warning "Failed to set timezone"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE ai_translator TO ai_translator;" || print_warning "Failed to grant privileges"

# Fix PostgreSQL schema permissions (CRITICAL FIX)
print_info "Fixing PostgreSQL schema permissions..."
sudo -u postgres psql -d ai_translator -c "GRANT ALL PRIVILEGES ON SCHEMA public TO ai_translator;" 2>/dev/null || print_warning "Failed to grant schema privileges"
sudo -u postgres psql -d ai_translator -c "GRANT CREATE ON SCHEMA public TO ai_translator;" 2>/dev/null || print_warning "Failed to grant create privilege"
sudo -u postgres psql -d ai_translator -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ai_translator;" 2>/dev/null || print_warning "Failed to alter default privileges"

# Create virtual environment
print_info "Creating Python virtual environment..."

# Use system Python without trying specific versions
...
# Install from requirements file with multiple fallbacks
...
# Try requirements_github.txt as another fallback
...
# Install essential packages manually if all failed
...
nginx -t && systemctl restart nginx || print_warning "Nginx configuration test failed, but continuing"
systemctl enable nginx || print_warning "Failed to enable Nginx, but continuing"

# Enable and start AI Translator service
print_info "Starting AI Translator service..."
systemctl daemon-reload || print_warning "Failed to reload daemon, but continuing"
systemctl enable ai-translator || print_warning "Failed to enable AI Translator service, but continuing"
systemctl start ai-translator || print_warning "Failed to start AI Translator service, but continuing"
...
if systemctl is-active --quiet ai-translator; then
    ...
    journalctl -u ai-translator --no-pager -n 10 || print_warning "Failed to check logs, but continuing"
fi
...
if ! command -v ollama &> /dev/null; then
    if curl -fsSL https://ollama.ai/install.sh | sh; then
        ...
        print_warning "Failed to install Ollama, but continuing"
    fi
else
...
if command -v ollama &> /dev/null; then
    ...
    ollama pull mistral || print_warning "Failed to pull mistral model, but continuing"
else
    print_warning "Ollama not found, skipping model pull"
fi
